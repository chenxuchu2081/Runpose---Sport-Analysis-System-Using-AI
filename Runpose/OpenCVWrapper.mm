                  ...
                  angleSet:(double*) angleSet
{
    ...
    for(int i = 0; i < keypoint_size; i++)
    {
        ...
        if(i==2 && angleSet[0]!=-1000){
            cv::ellipse(mat, point, cv::Point(70,70), angle, 0, angleSet[0], Scalar(255,0,0,255),FILLED);
        }

        if(i==4 && angleSet[1]!=-1000){
            cv::ellipse(mat, point, cv::Point(70,70), angle, 0, angleSet[1], Scalar(0,0,0,255),FILLED);
        }

        if(i==7 && angleSet[2]!=-1000){
            cv::ellipse(mat, point, cv::Point(70,70), angle, 0, angleSet[2], Scalar(255,0,255,255),FILLED);
        }

        if(i==10 && angleSet[3]!=-1000){
            cv::ellipse(mat, point, cv::Point(70,70), angle, 0, angleSet[3], Scalar(255,255,0,255),FILLED);
        }
